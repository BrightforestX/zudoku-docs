...
<Tabs>
  ...
  <Tab title="Data Pipeline">
    ``` Create a daily ETL pipeline that loads raw transactions from S3, applies feature
    engineering, and writes to feature store. ```
  </Tab>
  <Tab title="Monitoring">
    ``` Monitor my recommendation model's accuracy, latency, and drift over the past week, alert if
