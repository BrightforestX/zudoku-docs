...
```json
{
  ...
  "inputSchema": {
    ...
    "properties": {
      "task_type": {
        "type": "string",
        "enum": ["text_generation", "image_generation", "embeddings", "classification", "object_detection"],
...
<Tabs>
  <Tab title="Model Selection">
    ```
    Recommend a text generation model with less than 100ms latency,
    under $5 per 1M tokens, deployable to AWS Lambda.
    ```
  </Tab>
  <Tab title="Pipeline Design">
    ```
    Design a two-stage pipeline: use a small model for simple queries,
    fallback to GPT-4 for complex ones, with caching enabled.
    ```
  </Tab>
  <Tab title="Comparison">
    ```
    Compare GPT-4, Claude 3.5, and Llama 3.1 on code generation benchmarks
    focusing on accuracy and cost.
    ```
  </Tab>
</Tabs>
