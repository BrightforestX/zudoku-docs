...
The **GetDIYRPA.com RPA Workflow Agent** builds automation recipes, creates integration connectors, and orchestrates robotic process automation workflows without code.

## Overview

This agent specializes in workflow automation, helping users automate repetitive tasks and integrate disparate systems through visual workflow design and pre-built connectors.
  ...
  "inputSchema": {
    ...
    "properties": {
      ...
      "trigger": {
        "type": "object",
        "properties": {
          "type": {"type": "string", "enum": ["schedule", "webhook", "email", "file_upload", "manual"]},
          "config": {"type": "object"}
        }
      },
      "steps": {
...
<Tabs>
  <Tab title="Create Workflow">
    ```
    Create a workflow that sends a Slack message when a new customer
    signs up, adds them to our CRM, and sends a welcome email.
    ```
  </Tab>
  <Tab title="Add Connector">
    ```
    Add a Google Sheets connector to my data sync workflow,
    append new rows from form submissions every hour.
    ```
  </Tab>
  <Tab title="Schedule">
    ```
    Schedule my daily report workflow to run at 9 AM EST every
    weekday, email results to the team.
    ```
  </Tab>
</Tabs>
